...
图元坐标约定：胶囊的线段沿局部 Z 轴，参数为半径和线段半长；平面代表局部 `z <= 0` 的实体半空间，外法线为局部 +Z。刚体、多关节体、XPBD 粒子和 MPM 网格查询使用相同约定。USD 胶囊的 `axis = X/Y/Z` 在导入时与作者给定的旋转合成；无需调用方补偿轴向。

平面的碰撞范围没有横向尺寸或穿透深度上限，粗检测不能用薄层代替半空间。当前 CUDA broadphase 使用覆盖有限坐标范围的保守包围盒，窄检测再判断真实接触。渲染及相机／雷达使用有限的 XY 四边形，顶点位于局部 `z = 0`；显示尺寸不限制碰撞范围。上述离散接触规则不等同于 CCD。
