...
- 代码中的 $K$ 是 Tait 系数，参考态切线体积模量为 $\gamma K$；参考声速 $c_0 = \sqrt{\gamma K/\rho_0} \approx 37.42\ \mathrm{m/s}$。压缩时 $c = c_0 J^{-(\gamma-1)/2}$ 增大，CFL 还需计入流速，不能仅凭参考声速证明整段稳定。
...
4. **多子步 + 域约束**：`substeps = 40` 时参考声速的 $c_0\,dt_{\mathrm{sub}}/dx \approx 0.3543$，不是 0.09；局部压缩及流速会进一步收紧显式稳定条件。网格 xy 分离壁 BC 和地板平面 BC 构成池边界，允许波纹反射。
...
|---|---|---|
| dx | 0.011 | 0.025 |
| substeps / dt | 40 / $\tfrac{1}{240}$ s，参考波速 $c_0\,dt_{\mathrm{sub}}/dx = 0.3543$；稳定性另计局部声速与流速 | 20（需按材料波速检查显式稳定性） |
