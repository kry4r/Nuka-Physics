...
最新执行要求：用户允许动态夹爪先以视觉预览发布两支视频、替换主页布料展示并提交推送，随后优先补齐六种驱动模式。之后接续完整管线性能/调度/显存、可微基础与参数/轨迹优化、连续布面/自碰撞/CCD 及提袋/拧毛巾、PBF/MPM/柔体直接耦合与多材料界面。可微基础可在依赖就绪时提前。完整范围仍为 [引擎完善目标](2026-09-08-engine-completion-goal-zh.md) 和原 spec 剩余项，Editor 暂缓。

最新安排覆盖此前“提交后暂停”：视频供用户过目，发布后继续六种驱动模式。视觉预览不关闭空间精度及收敛 TODO，失败基线和细网格断点保留。
...
- [x] MPM 预测/交换/提交调度拆分完成；完整生产管线、公共接口/reset/渲染、内存检查及两支 demo 的全长对照通过，见 [调度报告](../research/2026-09-12-mpm-coupling-stages-zh.md)。
- [x] 有限质量多 owner 网格接触接入共同求解器；独立守恒/残差、完整生产管线、公共 API 和定向内存检查通过，见 [有限质量报告](../research/2026-09-12-finite-mass-grid-contact-zh.md)。
- [x] 动态 Panda 夹爪全景/近景视觉预览及主页替换：337 个状态、2.8 s 模拟、5 倍慢动作；保留 3.996 mm 侵入超过 2 mm 门及三类收敛未完成的状态。
- [ ] 补齐六种驱动模式的公共参数、真实控制律、导入/保存、状态读出、reset 和完整管线验收。OSC 必须支持通用 C/Python 创建入口和普通场景，覆盖多 articulation 任务缓冲区及超过 18 自由度的合法模型。

[压缩 demo](../research/2026-09-11-elastoplastic-compression-demo-zh.md) 使用给定位姿的压板，展示加载、卸载和残余形变；[bunny demo](../research/2026-09-11-elastoplastic-bunny-demo-zh.md) 展示带载压痕。动态 ABA 夹爪已使用真实有限力驱动及多 owner 共同接触，空间精度与收敛验收仍待完成。

## 后续功能清单（顺序已确认）

动态夹爪视觉预览发布后，先完成六种驱动模式，再针对通过质量门的新物理基线优化完整管线、调度与显存。可微基础可提前独立交付：先补生产前向/replay/checkpoint，再做参数或轨迹优化，接触反传依赖实际共同求解器。下表顺序已由用户确认，各项完成情况以独立验收记录为准。
...
| 2 · 六种驱动模式 | PD_POSITION、TORQUE、VELOCITY、COMPUTED_TORQUE、OSC、ACTUATOR 的公共输入、真实控制律和生命周期补齐 | 审计现有路径；共同质量算子与接触路径保持一致，验证解析响应、饱和/被动阻尼、多环境、reset、eager/graph 和 robot-cloth-fluid 完整管线 |
| 3 · 完整管线性能 | 测量并减少 eager 完成同步、host 提交、空网格/row/endpoint 工作；优化 Jacobian/质量算子、材料谱分解、邻域、岛调度和内存复用 | 物理变化后重新冻结通过质量门的分母；每项区分架构、CUDA 实现或物理算法。完整 pipeline 做五个独立进程对照，记录 GPU 完成时间、墙时、吞吐、显存和回退；不降低精度或迭代预算 |
| 4 · 可微基础，可提前 | 录制/重放委托生产 World，完整 checkpoint 和 backward 后状态恢复；补隐式阻尼、浮基/多根和实际关节变换伴随，去掉静默 24-link 限制；交付参数辨识或轨迹优化示例 | 先验证无接触且受支持的完整生产前向，有限差分步长扫描、checkpoint 重放身份、backward 不污染世界；明确支持矩阵。sticking/sliding/开合接触梯度随后按活动集与非光滑边界验收，详见 [审计](../research/2026-09-12-differentiable-contract-review-zh.md) |
| 5 · 连续柔体与操作 demo | 布面/软体点面、边边接触及自碰撞，厚度、摩擦、CCD 和投影后几何刷新；真实提袋装球、双夹爪拧毛巾 | 使用真实面/边插值端点和共同质量契约。验收漏球/穿透、应变、自碰撞、滑移、负载/扭矩与释放；输出完整视频。湿毛巾挤水须另有液体质量收支 |
| 6 · 直接多介质耦合 | 柔体↔PBF、柔体↔MPM、PBF↔MPM 直接交换，去掉全局组合模式限制；明确 MPM 多材料混合、分离、粘附规则 | 接续连续表面和多端点。按全量耦合 spec 的 15 类系统对逐项记录双向冲量、质量与能量收支、时间层和 reset/readout；共存或经中间刚体传力不算直接耦合 |
...
下列清单保留已完成内容、失败分母和剩余细项，执行顺序以上方已确认清单为准。相关模块成批修改、统一验证；性能结果只适用于各报告的物理输入。
...
- [ ] 收口动态夹爪的空间精度与三类收敛；共同接触和视觉预览已交付，材料落地画面不替代夹持验收。补充旧板压约 4.007 mm 穿入及薄指垫穿越诊断。
...
- [x] 用户已确认调度提交后的五项顺序，当前开始有限质量多体耦合与真实动态夹爪；可微基础可提前。完整矩阵与原 spec 剩余项保留，物理变化后重新冻结性能分母。
