...
状态：整体未完成。创建：2026-09-08；更新：2026-09-12。用户已确认 [后续功能 TODO](2026-09-08-demo-and-engine-todo-zh.md#后续功能清单顺序已确认) 的顺序：有限质量多体耦合与真实动态夹爪视觉预览 → 六种驱动模式 → 完整管线性能/调度/显存 → 可微基础与参数/轨迹优化（可提前）→ 连续布面/自碰撞/CCD 及提袋/拧毛巾 → PBF/MPM/柔体直接耦合与多材料界面。视觉预览发布后优先补齐六种驱动模式，Editor 暂缓。

用户最新要求：动态夹爪可以先以视觉预览渲染两支视频、替换主页布料展示并提交推送，供用户过目；之后优先补齐六种驱动模式，再继续其他工作。此安排覆盖此前“提交后暂停”，空间精度与收敛失败仍保留。
...
2026-09-12 当前交付：凸覆盖自动 cook、共享几何/owner、公共子步、并行查询及渲染提速已验收；Hencky J2 材料和两支弹塑性视频/主页已发布。运行时参数/tape 边界已修复，完整弹塑性步计时已加入，材料算术候选未采纳。MPM 预测/交换/提交调度已由 `775d168` 提交；有限质量共同接触的生产管线与独立不变量已通过，动态夹爪发布视觉预览；空间精度门失败，三类收敛待完成。最新顺序在性能前加入六种驱动模式。完整耦合矩阵、提袋/拧毛巾和其余 spec 范围保留。

用户最新补充将当前耦合范围明确为 **ABA 机器人、刚体、柔体（布料/体积软体）、XPBD/PBF 与 MLS-MPM 的全部相互作用**。按 [全量耦合细化 spec](../plans/2026-09-10-full-coupling-detailed-spec-zh.md) 检查每对系统的几何、端点、有效质量、时间层、反作用及公共接口；同场共存不等于已经耦合。共享几何/owner 是首批基础，不能以 MPM↔刚体或 MPM↔XPBD 的局部通过关闭总体目标。

2026-09-10 几何补充：非凸面支持不限定为凸分解。调研 GPU BVH/更新、连续点面/边边、距离界、RTX 和面简化；CPU 方法同样可作为 GPU 并行实现的参考。参照 PaMO/CoACD 及新的公开工作，独立探索“误差驱动的并行半空间分解”，先验证覆盖/孔洞/误差和算法成本，再接入生产。见 [调研与候选设计](../research/2026-09-10-mesh-collision-methods-zh.md) 和 [实施计划](../plans/2026-09-10-gpu-convex-decomposition-candidate-zh.md)。保留原始三角拓扑，不自动封闭薄袋/毛巾或把近似碰撞等同于原几何；新的算法/架构与 CUDA 调优分别归类。

自动 cook 的已交付范围见 [凸覆盖验收](../research/2026-09-10-automatic-convex-cover-validation-zh.md)：几何准备、设备选择、分解、缓存及统一消费者已集成；凸覆盖只组织原三角 BVH，预算不足时保留精确表面。它不等价于严格覆盖认证，也不替代变形表面的连续碰撞。

弹塑性体工作按 [细化 spec](../plans/2026-09-10-elastoplastic-body-detailed-spec-zh.md) 执行：调研、Hencky J2 材料、Fe/Fp/alpha 生命周期、公共 cook/reset/readout 和连续动态表面已按 [材料验收](../research/2026-09-11-elastoplastic-methods-zh.md) 收口。压缩和 bunny 视频由 `7c3da5d` 发布并放到主页第三行，Go2 技巧第二行、行走/布料最后一行。压缩使用给定位姿的压板，bunny 是带载压痕；动态 ABA 夹爪加载/卸载及有限质量共同接触仍须单独验收。
...
真实三角表面批次已按 [验收记录](../research/2026-09-10-triangle-surface-validation-zh.md) 收口：原始非凸/开放几何、BVH、三个接触消费者与序列化/recook 已接线；155 项导入、63 项场景、固定 E16、C/Python/reset/相机/渲染及定向 memcheck 通过，最终源码与二进制已冻结。自动 GPU 凸分解仍需生产集成；局部大 J、瞬时穿入、连续柔体与有限质量反馈仍未解决，整体目标保持 active。

最终新增两个正式 demo 验收：ABA 机器人通过真实夹爪接触提起装有多个动态小球的薄塑料袋；双夹爪抓持毛巾两端并相向扭转、停留和释放。要求连续表面碰撞、自碰撞、摩擦/滑移、负载与扭矩反馈以及完整渲染。不能以动画、隐藏固定点或中间刚体替代真实耦合。湿毛巾若展示挤水，必须实现并量测相应液体/多孔介质交换，不能以粒子特效代替。
...
| 已发布 | 两支弹塑性 demo 与主页 | 给定位姿压板的加载/卸载、bunny 带载压痕及精选视频/GIF；未完成动态 ABA 夹爪 |
| 当前提交 | MPM 预测/交换/提交调度 | 网格状态跨共同求解器保留，粒子/历史一次提交，详见 [报告](../research/2026-09-12-mpm-coupling-stages-zh.md) |
| 预览交付 | 有限质量共同接触及动态夹爪 | 多 owner 与同 articulation 交叉项、生产管线、独立不变量通过；视频替换布料卡片，3.996 mm 侵入超过 2 mm 门及收敛 TODO 保留 |
| 下一项 | 六种驱动模式 | 补齐 PD_POSITION、TORQUE、VELOCITY、COMPUTED_TORQUE、OSC、ACTUATOR 的生产控制律、公共输入/读出与生命周期 |
| 顺序已确认 | 完整管线性能与可微基础 | 性能按同质量五进程及显存对照；可微可提前，先统一生产前向/replay、checkpoint 和隐式阻尼伴随，再扩大接触与优化示例 |
| 后续 | 原 spec 其余引擎工作 | 完整耦合矩阵、连续表面/自碰撞、提袋/拧毛巾、直接多介质、已知物理失败、异构/关节、错误/API/传感器与隐式 MPM，依确认顺序推进 |
...
自动 GPU 辅助凸覆盖 cook 已收口，见 [验收](../research/2026-09-10-automatic-convex-cover-validation-zh.md)。同次切分合并查询、父裁剪和边拓扑复用，使真实杯子五进程冷 cook 中位数 2063.324 → 544.751 ms；缓存、预算回退、独立几何审查、完整管线/公共 API/reset/渲染和 memcheck 通过。仅精确表面 cook 仍明显更便宜；E16 运行步时/内存持平，不宣称求解加速或严格覆盖认证。剩余共享质量、完整介质耦合和操作 demo 按最新优先级接续，整体目标保持 active。
...
表中按依赖组织范围，实际执行优先级为：初步耦合/性能收口 → 渲染提速 → 弹塑性体调研、集成和真实夹爪 demo/主页替换 → 优先性能优化与多体耦合补齐 → 依赖相关的其余 TODO。模块 spec 的物理和接口验收要求仍有效。发现跨模块共因时成批修复；必要的首次反例和失败定位可独立运行。
...
- Commit 简洁、无合作者，包含 `[skip ci]`，按用户授权推送已完成改动。会话笔记、诊断临时脚本、逐帧输出、Git bundle 和下载工具不进入版本跟踪；本地复现证据放在 `.nuka-runs/` 或 `out/`。Editor 等引擎完善后再设计。

## 性能模块的重点

已收口并采纳 [MPM cell 共享传输](../research/2026-09-10-mpm-cell-transfer-validation-zh.md)：原 bunny-water 五对落水 eager 25.2728 → 20.2116 ms、graph 20.5661 → 11.6711 ms；Data arena 增加 47,692,800 B。最终版独立物理门、water/jelly、E=16 pipeline、memcheck/synccheck 和 8 对渲染帧通过。jelly 场景墙时中位减少 5.70%，小水池增加 8.13% 且逐对波动；中间版压痕回退 12.88% 保留且暂停追测。按用户最新授权进入耦合，未宣称这些回退已解决或全部性能目标完成。
...
当前 MLS-MPM 性能收口后接续完整多体/多介质耦合：无 SDF collider 的通用几何查询，多个碰撞端点的有限质量约束，刚体/关节子步反作用与速度刷新，以及 MPM↔XPBD/其他介质的完整交换链路。SDF 是几何表示之一，不应成为通用耦合的必需条件；解析 primitive、凸体和网格查询生成同一接触契约，不增设场景求解器。当前 bunny 的 SDF、独立网格地板、single owner 和延迟 articulation deposit 等边界必须随性能结果报告，不以未覆盖能力无限推迟已有有效路径的优化，也不把局部性能验收当作完整多体耦合。耦合完成后继续原 spec 的其余功能、API、高级能力与剩余性能项。
