...
2026-09-10 当前 MLS-MPM 性能批次已收口，正在推进凸分解自动 cook 与完整多体/多介质耦合。按最新要求，完成这两项后继续 MLS-MPM 修复，再插入弹塑性体调研、集成与机械臂挤压 demo，渲染验收并替换主页 Go2 行走视频后，返回原 spec 的其余工作。主要验证 MLS-MPM water pool（原 bunny-water）和 jelly，暂停压痕 demo 测试。按用户最新授权，不以穷尽所有性能热点作为转阶段前提；其余性能项与历史回退保留 TODO。总范围和 active 状态不变。
...
| 当前 | GPU 凸分解自动 cook、全量耦合与两项操作 demo | 按全量矩阵推进 ABA/刚体/柔体/XPBD/PBF/MLS-MPM；共享几何/owner → 有限质量与时间层 → 连续表面/自碰撞 → 直接介质交换 → 提袋装球和拧毛巾。以守恒、界面残差、抓持/释放和步长收敛验收 |
| 接续 1 | MLS-MPM 相关修复 | 复查局部大 J、本构/体积、材料状态、边界和耦合残留问题，以 water pool/jelly 与完整 pipeline 验收；保留失败，物理修复后重建性能分母 |
| 接续 2 | 弹塑性体网络调研与通用集成 | 比较 MPM/FEM 等大变形弹塑性方法，明确屈服、流动、硬化、塑性状态和数值积分；分别验收共享契约、物理算法与后端实现 |
| 接续 3 | 机械臂挤压 demo、渲染和主页替换 | 真实夹爪挤压物体，展示弹性恢复、屈服及卸载残余形变，记录反力/耗散；视频和 GIF 验收后替换 Go2 行走展示 |
| 接续 4 | 原 spec 的其余引擎工作 | 接续异构映射、接触/几何、错误契约、API/传感器、可微及其他尚未验收项，持续回写细化规格和实际进度 |
...
表中按依赖组织范围，实际执行优先级以前述最新顺序为准：凸分解自动 cook 与完整耦合 → MLS-MPM 修复 → 弹塑性体调研与集成 → 机械臂挤压 demo 渲染及主页替换 → 原 spec 其余项及剩余性能。模块 spec 的依赖和验收要求仍然有效。发现跨模块共因时成批修复；必要的首次反例和失败定位可独立运行，不为维持批次而保留已知错误。
