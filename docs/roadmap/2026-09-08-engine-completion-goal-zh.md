...
状态：active。创建：2026-09-08；更新：2026-09-11。依据用户最新要求，后续成批修改多个相关模块，再统一验证；Editor 暂缓。
...
2026-09-11 最新顺序：初步耦合与性能收口，**至少先完成渲染提速**，随后进行弹塑性体调研、通用集成与真实夹爪挤压 demo，渲染验收并替换主页 Go2 行走展示。**demo 后继续以性能优化和多体耦合补齐为优先**，其余 TODO 按两者依赖推进。凸覆盖自动 cook、共享几何/owner、公共子步和并行表面查询已验收；完整耦合矩阵、提袋装球及拧毛巾保留，按最新优先级接续。弹塑性 demo 所需的有限质量反馈、材料正确性和生命周期仍须完成。主要介质验证保持 water pool/jelly，压痕和 Editor 暂缓；总范围与 active 状态不变。
