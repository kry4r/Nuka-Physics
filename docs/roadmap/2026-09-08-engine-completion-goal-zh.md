...
状态：整体未完成。创建：2026-09-08；更新：2026-09-12。按用户最新要求，完成当前 MPM 调度批次的验证与 commit/push 后，先确认 [后续功能 TODO](2026-09-08-demo-and-engine-todo-zh.md#后续功能清单待确认)，再开启下一批；Editor 暂缓。以下历史排程不覆盖这项确认要求。
...
2026-09-12 当前交付：凸覆盖自动 cook、共享几何/owner、公共子步、并行查询及渲染提速已验收；Hencky J2 材料和两支弹塑性视频/主页已发布。运行时参数/tape 边界已修复，完整弹塑性步计时已加入，材料算术候选未采纳。本批完成 MPM 预测/交换/提交调度；有限质量共同接触、完整管线性能和可微基础作为下一批候选，先由用户确认顺序。完整耦合矩阵、提袋/拧毛巾和其余 spec 范围保留。
...
| 待确认 | 有限质量共同接触及动态夹爪 | 多 owner 网格端点接入 ContactBlock，含同 articulation 交叉项，独立检查守恒/残差、反力、卸载恢复与残余形变 |
| 待确认 | 完整管线性能与可微基础 | 性能按同质量五进程及显存对照；可微先统一生产前向/replay、checkpoint 和隐式阻尼伴随，再扩大接触与优化示例 |
