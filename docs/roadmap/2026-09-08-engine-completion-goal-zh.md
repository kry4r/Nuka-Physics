...
2026-09-10 当前 MLS-MPM 性能批次已收口，开始分析、改造完整多体/多介质耦合，随后按原 spec 继续全部未完成工作。主要验证 MLS-MPM water pool（原 bunny-water）和 jelly，暂停压痕 demo 测试。按用户最新授权，不以穷尽所有性能热点作为转阶段前提；其余性能项与历史回退保留 TODO。总范围和 active 状态不变。
...
| 当前 | 全量耦合与两项操作 demo | 按全量矩阵推进 ABA/刚体/柔体/XPBD/PBF/MLS-MPM；共享几何/owner → 有限质量与时间层 → 连续表面/自碰撞 → 直接介质交换 → 提袋装球和拧毛巾。以守恒、界面残差、抓持/释放和步长收敛验收 |
...
表中按依赖组织范围，实际执行优先级以前述最新顺序为准：当前 MLS-MPM 性能收口 → 完整耦合 → 原 spec 其余项及剩余性能。模块 spec 的依赖和验收要求仍然有效。发现跨模块共因时成批修复；必要的首次反例和失败定位可独立运行，不为维持批次而保留已知错误。
