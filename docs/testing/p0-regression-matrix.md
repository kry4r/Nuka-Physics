...
| CudaParticleCouplingTiming/DeviceWorldBoxRigidImpulseCouplingUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA coupling passes over 4096 particles against a cooked dynamic `DeviceWorld` box with row-solver rigid linear/angular impulse feedback | Wall time | < 1000 ms |
