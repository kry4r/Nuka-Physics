...
| CudaParticleCouplingTiming/DeviceWorldMultiSlotDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked two-box corner coupling passes over 2048 particles, validating multiple active cache slots plus CUDA coupling-row solver diagnostics per particle | Wall time | < 1000 ms |
