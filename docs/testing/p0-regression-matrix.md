...
| CudaParticleWorld/IteratesDeviceWorldCouplingRowsWithCudaScheduler | `tests/runtime/test_cuda_particle_world.cpp` | Verifies configurable CUDA row-solver scheduling runs multiple row-solver sweeps per fixed step, reports iteration/kernel-launch counts, and preserves Coulomb friction row bounds | CUDA |
...
| CudaParticleCouplingTiming/DeviceWorldMultiSlotDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked two-box corner coupling passes over 2048 particles, validating multiple active cache slots plus three-sweep CUDA coupling-row solver scheduler diagnostics per particle | Wall time | < 1000 ms |
