...
| CudaParticleCouplingTiming/DeviceWorldWarmStartDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked `DeviceWorld` box coupling passes over 4096 particles with four-slot per-particle warm-start cache and force/torque diagnostics enabled | Wall time | < 1000 ms |
| CudaParticleCouplingTiming/DeviceWorldMultiSlotDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked two-box corner coupling passes over 2048 particles, validating multiple active cache slots plus CUDA coupling-row diagnostics per particle | Wall time | < 1000 ms |
