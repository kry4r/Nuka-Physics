...
| CudaBatchedWorld/SolvesContactsIndependentlyOnDevice | `tests/runtime/test_cuda_batched_world.cpp` | Verifies batched CUDA contact constraints solve against flattened device state while exposing the device `RigidConstraintBlock` row buffer and scheduler diagnostics | CUDA |
| CudaBatchedWorld/StepPipelineCanSolveBatchedContacts | `tests/runtime/test_cuda_batched_world.cpp` | Verifies `StepBatchedCudaWorld()` can run integration, batched broadphase/contact generation, batched contact solve, and aggregate scheduler diagnostics in one CUDA pipeline | CUDA |
...
| CudaBatchedWorld/AppliesVelocityDrivesIndependentlyOnDevice | `tests/runtime/test_cuda_batched_world.cpp` | Verifies batched CUDA joint/drive rows update angular velocity per instance from cooked tables while exposing scheduler row-buffer diagnostics | CUDA |
| CudaBatchedWorld/StepPipelineCanSolveBatchedJointsAndDrives | `tests/runtime/test_cuda_batched_world.cpp` | Verifies `StepBatchedCudaWorld()` can run batched integration plus the instance-parallel joint/drive fast path with post-solve scheduler diagnostics | CUDA |
...
| CudaParticleWorld/PersistsDeviceWorldCouplingNormalImpulseForWarmStart | `tests/runtime/test_cuda_particle_world.cpp` | Verifies CUDA cooked-shape coupling persists per-particle normal impulse and reports same-shape warm-start diagnostics on the next step | CUDA |
| CudaParticleWorld/ClearsDeviceWorldCouplingImpulseWhenContactSeparates | `tests/runtime/test_cuda_particle_world.cpp` | Verifies CUDA clears stale per-particle coupling impulse and shape index on the device when contact separates | CUDA |
| CudaParticleWorld/StoresMultipleDeviceWorldCouplingSlotsForOneParticle | `tests/runtime/test_cuda_particle_world.cpp` | Verifies one CUDA particle can keep two cooked rigid contacts in separate device cache slots with warm-start, active-slot, force, and torque diagnostics | CUDA |
| CudaParticleWorld/AssemblesDeviceWorldCouplingConstraintRowsOnCuda | `tests/runtime/test_cuda_particle_world.cpp` | Verifies active CUDA coupling slots also store GPU-resident constraint row data: particle/shape/body ids, normal, Jacobian, effective mass, position error, and normal impulse | CUDA |
| CudaParticleWorld/ExposesCudaConstraintRowBufferViewForScheduler | `tests/runtime/test_cuda_particle_world.cpp` | Verifies cooked particle/rigid coupling exposes the real CUDA row storage through a scheduler-facing row-buffer view with row family, row layout, schedule mode, device pointer, owner count, rows per owner, row count, and stride metadata | CUDA |
| CudaParticleWorld/ExecutesDeviceWorldCouplingRowsWithCudaSolverKernel | `tests/runtime/test_cuda_particle_world.cpp` | Verifies CUDA cooked particle/rigid coupling rows execute in a row-solver kernel that applies normal impulses to particle and rigid linear/angular velocities | CUDA |
| CudaParticleWorld/AccumulatesMultipleCudaCouplingRowsIntoOneParticleVelocity | `tests/runtime/test_cuda_particle_world.cpp` | Verifies one CUDA particle with multiple active cooked coupling rows accumulates every row impulse into particle velocity during the per-particle row-solver sweep | CUDA |
| CudaParticleWorld/IteratesDeviceWorldCouplingRowsWithCudaScheduler | `tests/runtime/test_cuda_particle_world.cpp` | Verifies configurable CUDA row-solver scheduling runs multiple row-solver sweeps per fixed step through the scheduler envelope, reports row kind/layout/schedule mode, owner/row counts, launch counts, active rows, finite normal/tangent delta and residual diagnostics, and preserves Coulomb friction row bounds | CUDA |
| CudaParticleWorld/RecordsMultiStepRowSolverDiagnosticsBeforeFixedSlotCap | `tests/runtime/test_cuda_particle_world.cpp` | Verifies multi-step CUDA row-solver calls record the first solver-sweep diagnostic slots without overwriting earlier steps and keep true iteration counts separate from fixed report storage | CUDA |
| CudaParticleWorld/SolvesDeviceWorldCouplingFrictionRowsOnCuda | `tests/runtime/test_cuda_particle_world.cpp` | Verifies CUDA cooked particle/rigid coupling rows solve tangent friction sub-rows in the row-solver kernel, clamp them by `friction * normal_impulse`, and reduce tangential speed | CUDA |
| CudaParticleWorld/WarmStartsDeviceWorldCouplingFrictionRowsOnCuda | `tests/runtime/test_cuda_particle_world.cpp` | Verifies CUDA cooked particle/rigid coupling rows persist tangent friction impulses across same-shape contacts and report tangent warm-start count/magnitude | CUDA |
| CudaParticleWorld/ReportsNoRowSolverDiagnosticsWhenCudaRowSolverDisabled | `tests/runtime/test_cuda_particle_world.cpp` | Verifies the direct-coupling transition path reports zero CUDA row-solver launch and impulse diagnostics when row solving is explicitly disabled | CUDA |
...
| CudaConstraintSolver/SolvesPlaneBoxContactOnDevice | `tests/runtime/test_cuda_solver.cpp` | Verifies CUDA assembles contact rows, exposes the device `ConstraintBlock` storage as `RigidConstraintBlock` scheduler rows, reports global row-sweep metadata/delta/residual diagnostics, and solves normal impulses plus position correction against a static plane | CUDA |
...
| CudaConstraintSolver/AppliesWorldAnchoredVelocityActuatorOnDevice | `tests/runtime/test_cuda_solver.cpp` | Verifies CUDA joint and actuator assembly drive a static-world-anchored joint through the rigid scheduler envelope with observable row metadata and finite residual diagnostics | CUDA |
...
| CudaBatchContactTiming/BatchedContactSolveUnderOneSecond | `tests/perf/test_cuda_batch_contact_timing.cpp` | 60 CUDA batched integration/contact/PGS passes over 128 plane-box environments sharing one cooked template, including scheduler metadata/residual checks | Wall time | < 1000 ms |
| CudaBatchJointDriveTiming/BatchedJointDriveSolveUnderOneSecond | `tests/perf/test_cuda_batch_joint_drive_timing.cpp` | 60 CUDA batched integration/joint projection/velocity-drive passes over 128 two-body environments using the instance-parallel joint/drive fast path plus scheduler diagnostics | Wall time | < 1000 ms |
...
| CudaParticleCouplingTiming/DeviceWorldRigidImpulseCouplingUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA coupling passes over 4096 particles against a cooked dynamic offset `DeviceWorld` sphere with row-solver rigid linear/angular impulse feedback | Wall time | < 1000 ms |
| CudaParticleCouplingTiming/DeviceWorldBoxRigidImpulseCouplingUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA coupling passes over 4096 particles against a cooked dynamic `DeviceWorld` box with row-solver rigid linear/angular impulse feedback and tangent friction-row diagnostics | Wall time | < 1000 ms |
| CudaParticleCouplingTiming/DeviceWorldCapsuleRigidImpulseCouplingUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA coupling passes over 4096 particles against a cooked dynamic `DeviceWorld` capsule with row-solver rigid linear/angular impulse feedback | Wall time | < 1000 ms |
| CudaParticleCouplingTiming/DeviceWorldWarmStartDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked `DeviceWorld` box coupling passes over 4096 particles with four-slot per-particle normal/tangent warm-start cache, row-solver impulse diagnostics, friction, and force/torque diagnostics enabled | Wall time | < 1000 ms |
| CudaParticleCouplingTiming/DeviceWorldMultiSlotDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked two-box corner coupling passes over 2048 particles, validating multiple active cache slots plus scheduler-level row metadata, active-row counts, launch counts, and finite three-sweep CUDA coupling-row solver delta/residual diagnostics per particle | Wall time | < 1000 ms |
| CudaSolverTiming/ContactAssemblyAndPgsUnderOneSecond | `tests/perf/test_cuda_solver_timing.cpp` | 30 CUDA broadphase/contact/constraint assembly/PGS passes over 128 cooked dynamic boxes and a static plane, validating the rigid constraint scheduler report under load | Wall time | < 1000 ms |
