...
| CudaParticleCouplingTiming/DeviceWorldWarmStartDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked `DeviceWorld` box coupling passes over 4096 particles with per-particle warm-start cache and diagnostics enabled | Wall time | < 1000 ms |
