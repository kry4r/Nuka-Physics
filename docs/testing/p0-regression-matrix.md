...
| CudaParticleWorld/ExposesCudaConstraintRowBufferViewForScheduler | `tests/runtime/test_cuda_particle_world.cpp` | Verifies cooked particle/rigid coupling exposes the real CUDA row storage through a scheduler-facing row-buffer view with row family, device pointer, owner count, rows per owner, row count, and stride metadata | CUDA |
...
| CudaParticleWorld/IteratesDeviceWorldCouplingRowsWithCudaScheduler | `tests/runtime/test_cuda_particle_world.cpp` | Verifies configurable CUDA row-solver scheduling runs multiple row-solver sweeps per fixed step, reports iteration/kernel-launch counts plus finite normal/tangent delta and residual diagnostics, and preserves Coulomb friction row bounds | CUDA |
