...
| CudaParticleCouplingTiming/DeviceWorldWarmStartDiagnosticsUnderOneSecond | `tests/perf/test_cuda_particle_coupling_timing.cpp` | 60 CUDA cooked `DeviceWorld` box coupling passes over 4096 particles with four-slot per-particle warm-start cache, row-solver impulse diagnostics, and force/torque diagnostics enabled | Wall time | < 1000 ms |
