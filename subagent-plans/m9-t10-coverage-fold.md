...
| 7 | SDF precision oracle (4/4 cell-tol) | `tests/collision/test_sdf_tier_wired.cpp` | **YES** — `#include narrowphase_dispatch.hpp` (+ tests `sdf_contact.hpp` `find_sdf_contact_newton` THROUGH the dispatch) | **★ FLAGGED-COVERAGE-GAP → M11 carry-forward (PREMISE CORRECTED by M9-end review 2026-06-14)** — the original ACCEPTABLE-LOSS verdict was WRONG: it assumed `narrowphase_dispatch.hpp` + `find_sdf_contact_newton` would be DELETED at step 13, but T11-collision (0fee020) KEPT them (the SDF/IFT contact-gradient family is deferred to M11 per owner ruling #4, NOT deleted). So this gate was dropped on a FALSE premise — the infra it tests is still LIVE. The lost slice (the dispatch handler geom_a/geom_b side-A normal-sign binding + the absolute box-on-box-vs-analytic manifold check, `BoxOnBoxMatchesAnalyticalNormalAndPenetration`) is PARTIALLY backstopped by the surviving `test_sdf_contact_adjoint_fd` (forward depth-vs-translation envelope). **M11 MUST re-assert the SDF-tier forward accuracy gate when the host narrowphase is migrated/integrated** (salvageable verbatim from `fcae2a6:tests/collision/test_sdf_tier_wired.cpp`). SDF *determinism* (the cook) survives separately (#16). |
...
2. **Delete outright (ACCEPTABLE-LOSS):**
   - ~~`test_sdf_tier_wired.cpp`~~ **PREMISE CORRECTED (M9-end review)**: its infra
     (`narrowphase_dispatch` + `find_sdf_contact_newton`) was KEPT, NOT deleted (SDF/IFT
     family → M11 per ruling #4). This is a **FLAGGED-COVERAGE-GAP → M11 carry-forward**
     (see item #7), partially backstopped by `test_sdf_contact_adjoint_fd`. M11 MUST
     re-assert the SDF-tier forward gate (salvage `BoxOnBoxMatchesAnalyticalNormalAndPenetration`
     from `fcae2a6`).
