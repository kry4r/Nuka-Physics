#pragma once
// ---------------------------------------------------------------------------
// G1 union-world SHARED test scaffolding (extracted G1e, test-side ONLY).
// ---------------------------------------------------------------------------
// The whole-body H1 UNION scene authoring + the PD/CoP drive seams the G1 parity
// TU (tests/coresident/test_batched_h1_union_world.cpp) and the G1e union
// throughput perf cases (tests/coresident/test_batched_unified_world_perf.cpp)
// BOTH build from. EXTRACTED VERBATIM (byte-for-byte) from the union TU's anon
// namespace at G1e so the two TUs share ONE authoring path instead of replicating
// the 200-step settle pre-roll / placement search / hand-frame cup carry / table-
// height derivation (which would drift). NOTHING here is production code; it is the
// exact scene/drive scaffolding the union parity gates were proven on.
//
// All symbols live in nuka::test::h1union. The union TU does
// `using namespace nuka::test::h1union;` inside its anon namespace so its existing
// unqualified references (kInvalidLink / BuildGraspStandTableScene / DrivePdOracle
// / ...) keep compiling unchanged -> the 13 union gates stay byte-identical.
// ---------------------------------------------------------------------------
...
inline const char* const kWrapDriven[] = {
...
// ===========================================================================
inline nuka::demo::Cooked LoadFloatingGraspCook(const std::string& mjcf) {
...
inline UnionScene BuildFloatingNoContactScene(bool grasp_cook = false) {
    UnionScene sc;
...
inline coresident::BatchedSceneTemplate MakeUnionTemplate(const UnionScene& sc) {
...
// ===========================================================================
inline UnionScene BuildFeetGroundScene(const nuka::phi::DeviceContext& context,
...
inline std::vector<DriveLink> BuildDriveSet(const nuka::demo::Cooked& h1) {
...
inline std::vector<DriveLink> BuildStanceHoldDriveSet(const UnionScene& sc) {
    std::vector<DriveLink> out;
    const auto legs = nuka::demo::ResolveLegLinks(sc.h1);
    for (uint32_t s = 0u; s < 10u; ++s) {
        const uint32_t j = s % 5u;
...
inline std::vector<DriveLink> BuildGraspStanceDriveSet(const UnionScene& sc,
...
inline uint32_t DofIndexOf(const articulation::ArticulationHostState& host,
...
inline void DrivePdOracle(coresident::UnifiedCoResidentStepper& stepper, const UnionScene& sc,
...
inline void DrivePdBatched(coresident::BatchedUnifiedWorld& world, const UnionScene& sc,
...
inline UnionScene BuildGraspStandScene(const nuka::phi::DeviceContext& context) {
...
inline UnionScene BuildGraspStandTableScene(const nuka::phi::DeviceContext& context) {
