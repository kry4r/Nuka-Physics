...
struct ContactStep {
    ...
    OwnedDeviceBuffer rows_buf, jn_buf, jt1_buf, jt2_buf, minv_buf;
    // Host readback.
...
ContactStep RunContactStep(cudaStream_t ctx, int ctx_dev,
    ...
    s.buffers = art::UploadArticulationState(ctx, ctx_dev, host);
    s.view = s.buffers.View();
    art::FeatherstoneAba::ComputeAccelerations(ctx, ctx_dev, s.view, 0.0f);
    ...
    s.minv_buf = OwnedDeviceBuffer(tile * sizeof(float));
    art::ComputeArticulationInertiaM(
        ctx, ctx_dev, s.view, max_dof,
        static_cast<art::LinkSpatialInertia*>(composite.Data()),
        static_cast<float*>(m_buf.Data()));
    art::FactorArticulationInertiaM(ctx, ctx_dev, s.view, max_dof,
    ...
    art::ComputeContactTangentBasis(ctx, ctx_dev,
    ...
    s.jn_buf = OwnedDeviceBuffer(jac_len * sizeof(float));
    s.jt1_buf = OwnedDeviceBuffer(jac_len * sizeof(float));
    s.jt2_buf = OwnedDeviceBuffer(jac_len * sizeof(float));
    art::ComputeContactChainJacobians(ctx, ctx_dev, s.view,
                                      static_cast<const uint32_t*>(link_buf.Data()),
    ...
    OwnedDeviceBuffer meff_n(slot_count * sizeof(float));
    OwnedDeviceBuffer meff_t1(slot_count * sizeof(float));
    OwnedDeviceBuffer meff_t2(slot_count * sizeof(float));
    art::ComputeContactEffectiveMass(ctx, ctx_dev, s.view,
                                     static_cast<const uint32_t*>(link_buf.Data()),
    ...
    s.rows_buf = OwnedDeviceBuffer(slot_count * sizeof(art::ArticulatedContactRow));
    art::AssembleArticulatedContactRows(
        ctx, ctx_dev, s.view, static_cast<const uint32_t*>(link_buf.Data()),
        static_cast<const Vec3*>(normal_buf.Data()),
...
IftOut RunIft(cudaStream_t ctx, int ctx_dev, ContactStep& s,
              const std::vector<float>& g, bool adaptive_routing = false) {
    const uint32_t n_link = s.view.total_link_count;
    OwnedDeviceBuffer g_buf(n_link * sizeof(float));
    g_buf.CopyFromHost(g.data(), n_link * sizeof(float));
    OwnedDeviceBuffer gqf_buf(n_link * sizeof(float));
    OwnedDeviceBuffer gbc_buf(static_cast<size_t>(s.view.articulation_count) * kMd *
                                  sizeof(float));
    ...
    diffsim::IftRunner runner(ctx, ctx_dev);
...
TEST(IftContactGradient, MatchesDenseReference) {
    ...
    ContactStep s = RunContactStep(ctx, ctx_dev, sc.host, sc.link, sc.point, sc.normal,
    ...
    const IftOut gpu = RunIft(ctx, ctx_dev, s, g);
    ...
    // Cross-check our A against the GPU kkt builder's A (downloaded).
    {
        ...
        diffsim::BuildContactDelassusSystem(
            ctx, ctx_dev, static_cast<const art::ArticulatedContactRow*>(s.rows_buf.Data()),
            static_cast<const float*>(s.jn_buf.Data()),
            static_cast<const float*>(s.jt1_buf.Data()),
            static_cast<const float*>(s.jt2_buf.Data()),
        ...
        // bufs is the production ContactDelassusBuffers (v2 Buffer*); settled by the
        // Synchronize above -> read via BufferDownload.
        nuka::phi::BufferDownload(bufs.values, Adev.data(), 0,
                                 Adev.size() * sizeof(float));
...
TEST(IftContactGradient, MatchesFiniteDifference) {
    ...
    ContactStep s0 = RunContactStep(ctx, ctx_dev, sc.host, sc.link, sc.point, sc.normal,
                                    sc.depth, sc.dt, sc.mu);
    ...
    const IftOut ift = RunIft(ctx, ctx_dev, s0, g);
    ...
    for (uint32_t k = 0u; k < s0.dof; ++k) {
        ...
        ContactStep sp = RunContactStep(ctx, ctx_dev, hp, sc.link, sc.point, sc.normal,
                                        sc.depth, sc.dt, sc.mu);
        ContactStep sm = RunContactStep(ctx, ctx_dev, hm, sc.link, sc.point, sc.normal,
                                        sc.depth, sc.dt, sc.mu);
    ...
    //    reference covers the full b_c vector. --
    {
        ...
        dp[0] += eps; dm[0] -= eps;
        ContactStep sp = RunContactStep(ctx, ctx_dev, sc.host, sc.link, sc.point, sc.normal,
                                        dp, sc.dt, sc.mu);
        ContactStep sm = RunContactStep(ctx, ctx_dev, sc.host, sc.link, sc.point, sc.normal,
                                        dm, sc.dt, sc.mu);
...
TEST(IftContactGradient, D1TwoRunByteExact) {
    ...
    const IftOut a = RunIft(ctx, ctx_dev, s, g);
    const IftOut b = RunIft(ctx, ctx_dev, s, g);
...
TEST(IftContactGradient, AdaptiveRoutingOffEqualsOnByteExact) {
    ...
    const IftOut off = RunIft(ctx, ctx_dev, s, g, /*adaptive_routing=*/false);
    const IftOut on = RunIft(ctx, ctx_dev, s, g, /*adaptive_routing=*/true);
...
TEST(IftContactGradient, BackwardRunnerSeamGate) {
    ...
    const IftOut direct = RunIft(ctx, ctx_dev, s, g);
    ...
    diffsim::RecomputeOrchestrator orch(ctx, ctx_dev, s.view, diffsim::RolloutParams{},
                                        zeros, zeros, zeros, mass);
    diffsim::BackwardRunner runner(ctx, ctx_dev, orch);
...
TEST(IftContactGradient, PerfIftVsPgs) {
    ...
    art::ArticulationDeviceBuffers buffers = art::UploadArticulationState(ctx, ctx_dev, host);
    art::ArticulationDeviceState view = buffers.View();
    art::FeatherstoneAba::ComputeAccelerations(ctx, ctx_dev, view, 0.0f);

    // Per-articulation M -> M^-1 (one tile per env, stride dof^2).
    OwnedDeviceBuffer composite(
        static_cast<size_t>(host.TotalLinkCount()) * sizeof(art::LinkSpatialInertia));
    OwnedDeviceBuffer m_buf(static_cast<size_t>(n_env) * tile * sizeof(float));
    OwnedDeviceBuffer minv_buf(static_cast<size_t>(n_env) * tile * sizeof(float));
    art::ComputeArticulationInertiaM(ctx, ctx_dev, view, max_dof,
                                     static_cast<art::LinkSpatialInertia*>(composite.Data()),
                                     static_cast<float*>(m_buf.Data()));
    art::FactorArticulationInertiaM(ctx, ctx_dev, view, max_dof,
                                    static_cast<const float*>(m_buf.Data()),
    ...
    auto up_u32 = [&](const std::vector<uint32_t>& v) {
        OwnedDeviceBuffer b(v.size() * sizeof(uint32_t));
        b.CopyFromHost(v.data(), v.size() * sizeof(uint32_t));
        return b;
    };
    auto up_v3 = [&](const std::vector<Vec3>& v) {
        OwnedDeviceBuffer b(v.size() * sizeof(Vec3));
    ...
    OwnedDeviceBuffer link_buf = up_u32(link);
    OwnedDeviceBuffer point_buf = up_v3(point);
    OwnedDeviceBuffer normal_buf = up_v3(normal);
    OwnedDeviceBuffer depth_buf = up_f(depth);

    OwnedDeviceBuffer t1_buf(slot_count * sizeof(Vec3));
    OwnedDeviceBuffer t2_buf(slot_count * sizeof(Vec3));
    art::ComputeContactTangentBasis(ctx, ctx_dev, static_cast<const uint32_t*>(link_buf.Data()),
    ...
    art::ComputeContactChainJacobians(ctx, ctx_dev, view, static_cast<const uint32_t*>(link_buf.Data()),
                                      static_cast<const Vec3*>(point_buf.Data()),
    ...
    OwnedDeviceBuffer meff_t1(slot_count * sizeof(float));
    OwnedDeviceBuffer meff_t2(slot_count * sizeof(float));
    art::ComputeContactEffectiveMass(ctx, ctx_dev, view, static_cast<const uint32_t*>(link_buf.Data()),
    ...
    OwnedDeviceBuffer rows_buf(slot_count * sizeof(art::ArticulatedContactRow));
    art::AssembleArticulatedContactRows(
        ctx, ctx_dev, view, static_cast<const uint32_t*>(link_buf.Data()),
        static_cast<const Vec3*>(normal_buf.Data()),
        static_cast<const float*>(depth_buf.Data()),
    ...
    OwnedDeviceBuffer g_buf = up_f(g);
    OwnedDeviceBuffer gqf_buf(view.total_link_count * sizeof(float));
    OwnedDeviceBuffer gbc_buf(static_cast<size_t>(n_env) * kMd * sizeof(float));
    ...
    auto time_ms = [&](auto&& fn, uint32_t reps) {
        ...
        cudaStreamSynchronize(ctx);
        cudaEventRecord(a, ctx);
        for (uint32_t r = 0u; r < reps; ++r) fn();
        cudaEventRecord(b, ctx);
    ...
    const double ms_pgs = time_ms(
        [&]() {
            art::SolveArticulatedContactRows(
                ctx, ctx_dev, view, static_cast<const art::ArticulatedContactRow*>(rows_buf.Data()),
                static_cast<const float*>(jn_buf.Data()),
                static_cast<const float*>(jt1_buf.Data()),
                static_cast<const float*>(jt2_buf.Data()),
