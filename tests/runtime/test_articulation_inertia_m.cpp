...
TEST(ArticulationInertiaM, CrbaMatchesKineticEnergyAndFactorizationIsDeterministic) {
    ...
    // ---------------------------------------------------------------------
    {
        ...
        nuka::phi::Buffer jac_buf(static_cast<size_t>(contact_count) * max_dof *
                                      sizeof(float),
                                  nuka::phi::MemoryKind::Device);
        nuka::phi::Buffer meff_buf(contact_count * sizeof(float),
                                   nuka::phi::MemoryKind::Device);
        ...
        // (the conditioning guard the spec explicitly requires, spec c).
        {
            ...
            nuka::phi::Buffer tiny_jac_buf(static_cast<size_t>(max_dof) * sizeof(float),
                                           nuka::phi::MemoryKind::Device);
