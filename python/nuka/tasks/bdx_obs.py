...
class BdxObsBuilder:
    ...
    def foot_bottom_z(self) -> torch.Tensor:
        """(N,2) each sole capsule's lowest world z (ankle link pose composed with
        the capsule local pose, minus the capsule radius)."""
        ...
        return world[:, :, 2] - self.foot_cap_radius              # (N,2)

    def foot_contact(self, ground_z: float = 0.0, thresh: float = 0.02) -> torch.Tensor:
        """(N,2) bool: a foot is loaded iff its sole bottom is within ``thresh`` of
        the ground -- the kinematic contact proxy (robust on flat ground; sidesteps
