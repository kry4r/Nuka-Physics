...
LIBERO_OBJECT_TASK = "pick up the orange juice and place it in the basket"
LIBERO_OBJECT_TARGET = np.array([0.05, -0.1, 0.035], dtype=np.float32)
LIBERO_OBJECT_BASKET = np.array([-0.00409625, 0.25071288, 0.035], dtype=np.float32)
LIBERO_OBJECT_AGENT_CAMERA_LOCAL = (
    0.0,
    0.0,
    0.0,
    1.0,
...
class LiberoBlackBowlController:
    ...
    def __init__(
        ...
        *,
        ...
        solver_max_pairs: int = LIBERO_SOLVER_MAX_PAIRS,
        target_reference: np.ndarray = LIBERO_TARGET_BOWL,
        place_reference: np.ndarray = LIBERO_PLATE,
        home: Sequence[float] = LIBERO_HOME,
        ...
        self.env_count = 2 if control_backend == "osc" else 1
        self.solver_contact_margin = float(solver_contact_margin)
        self.solver_max_pairs = int(solver_max_pairs)
        self.target_reference = np.asarray(target_reference, dtype=np.float32)
        self.place_reference = np.asarray(place_reference, dtype=np.float32)
        self.home = np.asarray(home, dtype=np.float32)
        if self.home.shape != (9,) or not np.isfinite(self.home).all():
            raise ValueError(f"expected finite LIBERO home state (9,), got {self.home.shape}")
        self.agent_camera_local = tuple(agent_camera_local)
        self.agent_camera_mount = int(agent_camera_mount)
    ...
    def reset(self, state: Sequence[float] | torch.Tensor | None = None) -> None:
        values = np.asarray(self.home if state is None else state, dtype=np.float32)
        ...
        if self.control_backend == "osc":
            self.task_local_pose[:] = torch.as_tensor(
    ...
    def attach_policy_cameras(self) -> None:
        ...
        self.world.attach_camera_sensor(
            self.agent_camera_mount,
            self.agent_camera_index,
            self.agent_camera_local,
            LIBERO_AGENT_FOV,
            LIBERO_CAMERA_SIZE,
            LIBERO_CAMERA_SIZE,
        )
    ...
    def camera_images(self) -> torch.Tensor:
        ...
        expected = (self.env_count, 2, LIBERO_CAMERA_SIZE, LIBERO_CAMERA_SIZE, 3)
...
class LiberoObjectController(LiberoBlackBowlController):
    """Panda controller configured for the official LIBERO floor object suite."""

    def __init__(self, scene: str | Path, device: nuka.Device, **kwargs):
        super().__init__(
            scene,
            device,
            home=LIBERO_OBJECT_HOME,
            target_reference=LIBERO_OBJECT_TARGET,
            place_reference=LIBERO_OBJECT_BASKET,
            agent_camera_local=LIBERO_OBJECT_AGENT_CAMERA_LOCAL,
            agent_camera_mount=nuka.SensorMount.BODY.value,
            agent_camera_index=0,
            **kwargs,
        )
...
__all__ = [
    ...
    "LIBERO_OBJECT_AGENT_CAMERA_LOCAL",
    "LIBERO_OBJECT_BASKET",
    "LIBERO_OBJECT_TARGET",
    "LIBERO_OBJECT_TASK",
