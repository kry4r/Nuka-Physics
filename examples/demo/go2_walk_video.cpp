...
constexpr const char* kDefaultBin = "out/go2_walk_trajectory.bin";
...
int main(int argc, char** argv) {
    ...
    // The orbiting follow-cam: eye/target off the dumped BASE (link 0), the orbit
    // angles a smooth function of walk progress `t` in [0,1].
    auto aim_camera = [&](uint32_t step) {
        ...
        // Azimuth: SIDE (-y flank, slightly toward the rear) -> FRONT-3/4 (ahead).
        // Stop at ~+80deg (a flattering front-3/4) rather than dead head-on so the
        // hero close keeps the dog's form, not a top-down plan view.
        const float phi = lerp(-0.14f, 0.44f * kPi, e);  // ~ -8deg -> ~+79deg
        // Elevation above the horizon: a soft crane that dips low mid-walk (close,
        // grounded) then settles LOW for a dramatic near-ground front hero. Kept
        // low overall so we see the dog's profile/face, not the top of its back.
        const float elev_deg = lerp(16.0f, 11.0f, e) +
                               5.0f * std::sin(e * kPi);  // gentle dip-then-settle
        const float elev = elev_deg * kPi / 180.0f;
        // Radius: eases in slightly so the dog grows as it arrives front-on.
        const float radius = lerp(2.30f, 2.05f, e);

        const float ch = std::cos(elev), sh = std::sin(elev);
        // Horizontal orbit direction in the xy-plane (phi=0 -> -y side).
        const float dx = std::sin(phi);
        const float dy = -std::cos(phi);
        opts.camera_eye = {b.x + radius * ch * dx,
                           b.y + radius * ch * dy,
                           b.z + radius * sh + 0.18f};
        // Aim at the body core (a hair above the base origin, toward the trunk).
        opts.camera_target = {b.x + 0.08f, b.y + 0.0f, b.z + 0.02f};
    };
