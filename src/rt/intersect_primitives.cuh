...
// fp64 normalize of a Vec3 (TRUE sqrt + TRUE divide, no rsqrt). Returns {0,0,0}
// for a (near-)zero vector. Host==device because the arithmetic is identical.
...
NUKA_RT_HD inline bool RayTriangleIntersect(const math::Vec3& origin,
    ...
    // Edges (fp64).
    const double e1x = static_cast<double>(v1.x) - static_cast<double>(v0.x);
    const double e1y = static_cast<double>(v1.y) - static_cast<double>(v0.y);
    const double e1z = static_cast<double>(v1.z) - static_cast<double>(v0.z);
    const double e2x = static_cast<double>(v2.x) - static_cast<double>(v0.x);
    const double e2y = static_cast<double>(v2.y) - static_cast<double>(v0.y);
    const double e2z = static_cast<double>(v2.z) - static_cast<double>(v0.z);

    const double dx = static_cast<double>(dir.x);
    const double dy = static_cast<double>(dir.y);
    const double dz = static_cast<double>(dir.z);
    ...
    if (a > -1.0e-12 && a < 1.0e-12) {
        return false; // ray parallel to triangle plane
    }
    const double f = 1.0 / a;

    // s = origin - v0
    const double sx = static_cast<double>(origin.x) - static_cast<double>(v0.x);
    const double sy = static_cast<double>(origin.y) - static_cast<double>(v0.y);
    const double sz = static_cast<double>(origin.z) - static_cast<double>(v0.z);
...
NUKA_RT_HD inline bool RaySphereIntersect(const math::Vec3& origin,
                                          ...
                                          float* out_u,
                                          float* out_v) {
    const double ocx = static_cast<double>(origin.x) - static_cast<double>(center.x);
    const double ocy = static_cast<double>(origin.y) - static_cast<double>(center.y);
    const double ocz = static_cast<double>(origin.z) - static_cast<double>(center.z);
    const double dx = static_cast<double>(dir.x);
    const double dy = static_cast<double>(dir.y);
    const double dz = static_cast<double>(dir.z);
    ...
    const double b = ocx * dx + ocy * dy + ocz * dz;       // half-b
    const double c = (ocx * ocx + ocy * ocy + ocz * ocz) - r * r;
    const double disc = b * b - a * c;                     // (half-b)^2 - a*c
    if (disc < 0.0) {
        return false;
    }
    ...
    *out_t = static_cast<float>(t);
    ...
    const double px = static_cast<double>(origin.x) + t * dx;
    const double py = static_cast<double>(origin.y) + t * dy;
    const double pz = static_cast<double>(origin.z) + t * dz;
    const math::Vec3 n{static_cast<float>(px - static_cast<double>(center.x)),
                       static_cast<float>(py - static_cast<double>(center.y)),
                       static_cast<float>(pz - static_cast<double>(center.z))};
    const math::Vec3 nn = RtNormalize(n);
    *out_normal = nn;
    ...
    const double uu = 0.5 + atan2(static_cast<double>(nn.z),
                                  static_cast<double>(nn.x)) / (2.0 * pi);
    const double vv = 0.5 - asin(static_cast<double>(nn.y) < -1.0 ? -1.0
                            : (static_cast<double>(nn.y) > 1.0 ? 1.0
                            : static_cast<double>(nn.y))) / pi;
...
NUKA_RT_HD inline bool RaySdfSphereTrace(const math::Vec3& origin,
    ...
    double aabb_tmin, aabb_tmax;
    {
        ...
        const double tx0 = (static_cast<double>(aabb.min.x) - static_cast<double>(origin.x)) * inv_x;
        const double tx1 = (static_cast<double>(aabb.max.x) - static_cast<double>(origin.x)) * inv_x;
        const double ty0 = (static_cast<double>(aabb.min.y) - static_cast<double>(origin.y)) * inv_y;
        const double ty1 = (static_cast<double>(aabb.max.y) - static_cast<double>(origin.y)) * inv_y;
        const double tz0 = (static_cast<double>(aabb.min.z) - static_cast<double>(origin.z)) * inv_z;
        const double tz1 = (static_cast<double>(aabb.max.z) - static_cast<double>(origin.z)) * inv_z;
        const double txmin = tx0 < tx1 ? tx0 : tx1, txmax = tx0 < tx1 ? tx1 : tx0;
        const double tymin = ty0 < ty1 ? ty0 : ty1, tymax = ty0 < ty1 ? ty1 : ty0;
        const double tzmin = tz0 < tz1 ? tz0 : tz1, tzmax = tz0 < tz1 ? tz1 : tz0;
        double a = txmin > tymin ? txmin : tymin; a = a > tzmin ? a : tzmin;
        double b = txmax < tymax ? txmax : tymax; b = b < tzmax ? b : tzmax;
        if (b < a || b < 0.0) {
            return false;
        }
        aabb_tmin = a > static_cast<double>(t_min) ? a : static_cast<double>(t_min);
        aabb_tmax = b;
    }
    ...
    for (int i = 0; i < max_iters; ++i) {
        ...
        const math::Vec3 p{static_cast<float>(static_cast<double>(origin.x) + t * static_cast<double>(dir.x)),
                           static_cast<float>(static_cast<double>(origin.y) + t * static_cast<double>(dir.y)),
                           static_cast<float>(static_cast<double>(origin.z) + t * static_cast<double>(dir.z))};
        math::Vec3 grad;
        ...
        double step = static_cast<double>(phi);
        if (step > static_cast<double>(max_step)) {
            step = static_cast<double>(max_step);
        }
        ...
        const double min_progress = static_cast<double>(max_step) * 1.0e-3;
