...
// ============================================================
//
// Reverse-mode matched pair for XPBDVolumeRow (dense_adjoint).
//
...
// Differentiable inputs + params for the xpbd_volume law.
struct XPBDVolumeAdjInputs {
    float C = 0.0f;
    float lambda = 0.0f;
...
xpbd_volume_unclamped_eval(const XPBDVolumeAdjInputs& in) {
    const float C = in.C;
...
__host__ __device__ __forceinline__ float
xpbd_volume_forward_eval(const XPBDVolumeAdjInputs& in) {
    const float u = xpbd_volume_unclamped_eval(in);
    return fminf(fmaxf(u, in.lower), in.upper);
}

// Clamp sub-gradient indicator: 1 strictly inside the band, else 0.
__host__ __device__ __forceinline__ float
xpbd_volume_clamp_active(const XPBDVolumeAdjInputs& in) {
    const float u = xpbd_volume_unclamped_eval(in);
...
__host__ __device__ __forceinline__ XPBDVolumeAdjGrads
xpbd_volume_adjoint_eval(const XPBDVolumeAdjInputs& in, float seed) {
    const float C = in.C;
    const float lambda = in.lambda;
    const float effective_mass = in.effective_mass;
    const float alpha_tilde = in.alpha_tilde;
    const float lower = in.lower;
    const float upper = in.upper;
    const float kClampActive = xpbd_volume_clamp_active(in);
