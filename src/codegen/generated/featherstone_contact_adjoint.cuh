...
// ============================================================
//
// Reverse-mode matched pair for FeatherstoneContactRow (stop_grad_on_event).
//
...
// Differentiable inputs + params for the featherstone_contact law.
struct FeatherstoneContactAdjInputs {
    float jv = 0.0f;
    float rhs = 0.0f;
    float lambda = 0.0f;
...
struct FeatherstoneContactAdjGrads {
    float grad_jv = 0.0f;
    float grad_rhs = 0.0f;
...
featherstone_contact_unclamped_eval(const FeatherstoneContactAdjInputs& in) {
...
__host__ __device__ __forceinline__ float
featherstone_contact_forward_eval(const FeatherstoneContactAdjInputs& in) {
    const float u = featherstone_contact_unclamped_eval(in);
...
__host__ __device__ __forceinline__ float
featherstone_contact_clamp_active(const FeatherstoneContactAdjInputs& in) {
    const float u = featherstone_contact_unclamped_eval(in);
...
__host__ __device__ __forceinline__ FeatherstoneContactAdjGrads
featherstone_contact_adjoint_eval(const FeatherstoneContactAdjInputs& in, float seed) {
    const float jv = in.jv;
    const float rhs = in.rhs;
    const float lambda = in.lambda;
    const float effective_mass = in.effective_mass;
    const float lower = in.lower;
    const float upper = in.upper;
    const float kClampActive = featherstone_contact_clamp_active(in);
