...
// ============================================================
// Source: tools/codegen/classes/xpbd_distance.yaml
// Regenerate: python tools/codegen/regen.py
// ============================================================
//
// Reverse-mode matched pair for XPBDDistanceRow (dense_adjoint).
//
...
// Differentiable inputs + params for the xpbd_distance law.
struct XPBDDistanceAdjInputs {
    float C = 0.0f;
    float lambda = 0.0f;
...
xpbd_distance_unclamped_eval(const XPBDDistanceAdjInputs& in) {
    const float C = in.C;
...
__host__ __device__ __forceinline__ float
xpbd_distance_forward_eval(const XPBDDistanceAdjInputs& in) {
    const float u = xpbd_distance_unclamped_eval(in);
    return fminf(fmaxf(u, in.lower), in.upper);
}

// Clamp sub-gradient indicator: 1 strictly inside the band, else 0.
__host__ __device__ __forceinline__ float
xpbd_distance_clamp_active(const XPBDDistanceAdjInputs& in) {
    const float u = xpbd_distance_unclamped_eval(in);
...
__host__ __device__ __forceinline__ XPBDDistanceAdjGrads
xpbd_distance_adjoint_eval(const XPBDDistanceAdjInputs& in, float seed) {
    const float C = in.C;
    const float lambda = in.lambda;
    const float effective_mass = in.effective_mass;
    const float alpha_tilde = in.alpha_tilde;
    const float lower = in.lower;
    const float upper = in.upper;
    const float kClampActive = xpbd_distance_clamp_active(in);

    XPBDDistanceAdjGrads g;
