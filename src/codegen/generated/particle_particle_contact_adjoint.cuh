...
// ============================================================
//
// Reverse-mode matched pair for ParticleParticleContactRow (dense_adjoint).
//
...
// Differentiable inputs + params for the particle_particle_contact law.
struct ParticleParticleContactAdjInputs {
    float C = 0.0f;
    float lambda = 0.0f;
...
struct ParticleParticleContactAdjGrads {
    float grad_C = 0.0f;
...
particle_particle_contact_unclamped_eval(const ParticleParticleContactAdjInputs& in) {
    const float C = in.C;
...
__host__ __device__ __forceinline__ float
particle_particle_contact_forward_eval(const ParticleParticleContactAdjInputs& in) {
    const float u = particle_particle_contact_unclamped_eval(in);
    return fminf(fmaxf(u, in.lower), in.upper);
}

// Clamp sub-gradient indicator: 1 strictly inside the band, else 0.
__host__ __device__ __forceinline__ float
particle_particle_contact_clamp_active(const ParticleParticleContactAdjInputs& in) {
    const float u = particle_particle_contact_unclamped_eval(in);
...
__host__ __device__ __forceinline__ ParticleParticleContactAdjGrads
particle_particle_contact_adjoint_eval(const ParticleParticleContactAdjInputs& in, float seed) {
    const float C = in.C;
    const float lambda = in.lambda;
    const float effective_mass = in.effective_mass;
    const float alpha_tilde = in.alpha_tilde;
    const float lower = in.lower;
    const float upper = in.upper;
    const float kClampActive = particle_particle_contact_clamp_active(in);
