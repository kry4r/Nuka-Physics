...
// ============================================================
//
// Reverse-mode adjoint kernel for XPBDDistanceRow (dense_adjoint).
...
__global__ void xpbd_distance_adjoint_kernel(
    ...
    uint32_t row_count)
{
    ...
    if (row.row_class_id != 6u) {
        return;
    }
    if (row.adjoint_kernel_id != 5u) {
        return;
    }
    ...
    XPBDDistanceAdjGrads g =
        xpbd_distance_adjoint_eval(in, seed_data[row_idx]);
