...
// ============================================================
//
// Reverse-mode adjoint kernel for MaximalJointRow (dense_adjoint).
...
__global__ void maximal_joint_adjoint_kernel(
    ...
    uint32_t row_count)
{
    ...
    if (row.row_class_id != 1u) {
        return;
    }
    if (row.adjoint_kernel_id != 2u) {
        return;
    }

    MaximalJointAdjInputs in;
    ...
    MaximalJointAdjGrads g =
        maximal_joint_adjoint_eval(in, seed_data[row_idx]);
