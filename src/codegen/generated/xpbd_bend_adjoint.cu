...
// ============================================================
//
// Reverse-mode adjoint kernel for XPBDBendRow (dense_adjoint).
...
__global__ void xpbd_bend_adjoint_kernel(
    ...
    uint32_t row_count)
{
    ...
    if (row.row_class_id != 7u) {
        return;
    }
    if (row.adjoint_kernel_id != 6u) {
        return;
    }
    ...
    XPBDBendAdjGrads g =
        xpbd_bend_adjoint_eval(in, seed_data[row_idx]);
