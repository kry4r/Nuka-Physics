// V3 finite-difference validation for the XPBDVolumeRow reverse-mode adjoint
// (v0.7 p09-B). XPBDVolumeRow is a dense_adjoint bilateral EQUALITY row (the box
// is unbounded -- clamp never active), so this harness mirrors the xpbd_distance
...
//
// The volume row's LOCAL projection law is IDENTICAL in form to the distance
// row's (the XPBD multiplier update is constraint-type-agnostic once the reduced
// scalars C/lambda/effective_mass/alpha_tilde are treated as independent inputs).
// The constraint-specific determinant GRADIENT is DOWNSTREAM of this local
// adjoint (it rides the position chain) and is validated separately, by a direct
// host FD check of grad C in the tet sim test -- NOT here.
//
...
struct XpbdVolumeFdConfig {
    uint32_t num_cases = 100u;
    uint64_t seed = 0xC0FFEEu;
    float fd_delta = 1.0e-2f;

    // Sample ranges. effective_mass = 1/(sum_i w_i|grad_i|^2 + alpha_tilde) is
    ...
    float C_abs_max = 0.5f;            // volume constraint value det(.) - 6*V_rest
...
// Away-from-boundary FD vs analytical-adjoint batch (XPBDVolumeRow).
