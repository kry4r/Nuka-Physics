...
open('.nuka-runs/_lbb3.py','w').write(src)
...
with nuka.Device.create(0) as device:
    ...
    w = c.world
    cf = torch.from_dlpack(w.buffer_view(nuka.CONTACT_FORCE))
    cl = torch.from_dlpack(w.buffer_view(nuka.CONTACT_LINK))
    print('CONTACT_FORCE', tuple(cf.shape), 'CONTACT_LINK', tuple(cl.shape), flush=True)
    ...
    def drive(g, grip, n, tag=None):
        ...
        if tag:
            f = cf.reshape(2,-1,3)[0].detach().cpu().numpy()
            l = cl.reshape(2,-1)[0].detach().cpu().numpy()
            print('%-8s grip %5.2fmm bowlz %.4f links %s |f| %s' % (
                tag, c.q[0,8].item()*1000, c.target_position()[2].item(),
                l.tolist(), np.round(np.linalg.norm(f,axis=-1),2).tolist()), flush=True)
